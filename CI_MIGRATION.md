...
Before we can upgrade to `quay.io/sameersbn/gitlab:8.1.0-2`, we need to assign the value of `GITLAB_CI_SECRETS_DB_KEY_BASE` (from CI) to `GITLAB_SECRETS_DB_KEY_BASE` in GitLab's environment.
...
### Upgrade to `quay.io/sameersbn/gitlab:8.1.0-2`

```bash
docker run -it --rm [OPTIONS] \
  --env GITLAB_CI_HOST=ci.example.com --env GITLAB_SECRETS_DB_KEY_BASE=xxxxxx \
  quay.io/sameersbn/gitlab:8.1.0-2 app:init
```
...
docker run -it --rm [OPTIONS] \
  --env GITLAB_CI_HOST=ci.example.com --env GITLAB_SECRETS_DB_KEY_BASE=xxxxxx \
  quay.io/sameersbn/gitlab:8.1.0-2 app:rake ci:migrate
```
