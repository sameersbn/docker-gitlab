...
When Keycloak is running, log in using the `Administration console`. You can visit the the Keycloak on the [local IP](http://localhost:10081) of your laptop.
...
Also make sure to use the GitLab image from this fork: `nuberabe/docker-gitlab:12.7.0`.
...
Visit the tab `Users` and click on `View all users` to modify the Admin user

![keycloak-users](images/keycloak-users.png)

Modify the fields `Email`, `First name` and `Last Name`.
![admin-account](images/keycloak-admin-acc.png)

Next, deploy GitLab, Reddis and Postgresql by running the following command: `docker-compose up -d gitlab redis postgresql`.
