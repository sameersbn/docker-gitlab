...
docker run --name gitlab -d [OPTIONS] \
    sameersbn/gitlab:8.7.2 app:sanitize
```
...
- **OAUTH_SAML_GROUPS_ATTRIBUTE**: Map groups attribute in a SAMLResponse to external groups. Comma separated list of quoted groups. Example: `'group1','group2'`. No defaults.
- **OAUTH_SAML_EXTERNAL_GROUPS**: List of external groups in a SAMLResponse. No defaults. 
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:8.7.2 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:8.7.2 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:8.7.2 app:rake gitlab:import:repos
```
...
> **Note**
>
> Upgrading to `sameersbn/gitlab:8.7.2` from `sameersbn/gitlab:7.x.x` can cause issues. It is therefore required that you first upgrade to `sameersbn/gitlab:8.0.5-1` before upgrading to `sameersbn/gitlab:8.1.0` or higher.
...
```bash
docker run --name gitlab -d [OPTIONS] sameersbn/gitlab:8.7.2
```
