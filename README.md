...
```bash
docker pull sameersbn/gitlab:10.7.3
```
...
docker run --name gitlab -d [OPTIONS] \
    sameersbn/gitlab:10.7.3 app:sanitize
```
...
| `OAUTH_SYNC_PROFILE_FROM_PROVIDER` | Sync user's profile from the specified Omniauth providers every time the user logs in. Defaults to `false`. |
| `OAUTH_SYNC_PROFILE_ATTRIBUTES` | Comma separated list of attributes which will sync from the providers above. (default: `email`). Available options are `name`, `email` and `location` |
...
> **Note**
>
> Upgrading to `sameersbn/gitlab:10.7.3` from `sameersbn/gitlab:7.x.x` can cause issues. It is therefore required that you first upgrade to `sameersbn/gitlab:8.0.5-1` before upgrading to `sameersbn/gitlab:8.1.0` or higher.
...
```bash
docker run --name gitlab -d [OPTIONS] sameersbn/gitlab:10.7.3
```
