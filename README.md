...
```bash
docker pull sameersbn/gitlab:6.9.0
```
...
docker run --name=gitlab -d \
  -v /opt/gitlab/data:/home/git/data \
  -v /opt/gitlab/mysql:/var/lib/mysql sameersbn/gitlab:6.9.0
```
...
docker run --name=gitlab -i -t --rm \
  ...
  -v /opt/gitlab/data:/home/git/data \
  sameersbn/gitlab:6.9.0 app:rake gitlab:setup
```
...
docker run --name=gitlab -i -t --rm [OPTIONS] \
  sameersbn/gitlab:6.9.0 app:rake gitlab:backup:create
```
...
```bash
docker run --name=gitlab -i -t --rm [OPTIONS] \
  sameersbn/gitlab:6.9.0 app:rake gitlab:backup:restore
```
...
```bash
docker run --name=gitlab -i -t --rm [OPTIONS] \
  sameersbn/gitlab:6.9.0 app:rake db:migrate
```

- **Step 5**: Start the image

```bash
docker run --name=gitlab -d [OPTIONS] sameersbn/gitlab:6.9.0
```
...
docker run --name=gitlab -d [OPTIONS] \
  sameersbn/gitlab:6.9.0 app:rake gitlab:env:info
```
...
```bash
docker run --name=gitlab -d [OPTIONS] \
  sameersbn/gitlab:6.9.0 app:rake gitlab:import:repos
```
