...
docker run --name=gitlab -it --rm [OPTIONS] \
  sameersbn/gitlab:7.4.2 app:rake gitlab:backup:create
```
...
```bash
docker run --name=gitlab -it --rm [OPTIONS] \
  sameersbn/gitlab:7.4.2 app:rake gitlab:backup:restore
```
...
```bash
docker run --name=gitlab -d [OPTIONS] sameersbn/gitlab:7.4.2
```
...
docker run --name=gitlab -d [OPTIONS] \
  sameersbn/gitlab:7.4.2 app:rake gitlab:env:info
```
...
```bash
docker run --name=gitlab -d [OPTIONS] \
  sameersbn/gitlab:7.4.2 app:rake gitlab:import:repos
```
