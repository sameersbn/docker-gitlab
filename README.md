...
Some secret variables in this list can be suffixed with `_FILE`, which will make the container entrypoint try to read the value from the specified path and set the environment variable to the content of the file. This is useful in environments where secrets are provided as files in `/run/secrets`, like Rancher 1.x... 

| Parameter | Description | _FILE support |
|-----------|-------------|---------------|
...
| `GITLAB_SECRETS_DB_KEY_BASE` | Encryption key for GitLab CI secret variables, as well as import credentials, in the database. Ensure that your key is at least 32 characters long and that you don't lose it. You can generate one using `pwgen -Bsv1 64`. If you are migrating from GitLab CI, you need to set this value to the value of `GITLAB_CI_SECRETS_DB_KEY_BASE`. No defaults. | YES |
| `GITLAB_SECRETS_SECRET_KEY_BASE` | Encryption key for session secrets. Ensure that your key is at least 64 characters long and that you don't lose it. This secret can be rotated with minimal impact - the main effect is that previously-sent password reset emails will no longer work. You can generate one using `pwgen -Bsv1 64`. No defaults. | YES |
| `GITLAB_SECRETS_OTP_KEY_BASE` |  Encryption key for OTP related stuff with  GitLab. Ensure that your key is at least 64 characters long and that you don't lose it. **If you lose or change this secret, 2FA will stop working for all users.** You can generate one using `pwgen -Bsv1 64`. No defaults. | YES |
...
| `GITLAB_ROOT_PASSWORD` | The password for the root user on firstrun. Defaults to `5iveL!fe`. GitLab requires this to be at least **8 characters long**. | YES |
...
| `GITLAB_ARTIFACTS_OBJECT_STORE_CONNECTION_AWS_ACCESS_KEY_ID` | AWS Access Key ID for the Bucket. Defaults to `AWS_ACCESS_KEY_ID` | YES |
| `GITLAB_ARTIFACTS_OBJECT_STORE_CONNECTION_AWS_SECRET_ACCESS_KEY` | AWS Secret Access Key. Defaults to `AWS_SECRET_ACCESS_KEY` | YES |
...
| `GITLAB_LFS_OBJECT_STORE_CONNECTION_AWS_ACCESS_KEY_ID` | AWS Access Key ID for the Bucket. Defaults to `AWS_ACCESS_KEY_ID` | YES |
| `GITLAB_LFS_OBJECT_STORE_CONNECTION_AWS_SECRET_ACCESS_KEY` | AWS Secret Access Key. Defaults to `AWS_SECRET_ACCESS_KEY` | YES |
...
| `GITLAB_UPLOADS_OBJECT_STORE_CONNECTION_AWS_ACCESS_KEY_ID` | AWS Access Key ID for the Bucket. Defaults to `AWS_ACCESS_KEY_ID` | YES |
| `GITLAB_UPLOADS_OBJECT_STORE_CONNECTION_AWS_SECRET_ACCESS_KEY` | AWS Secret Access Key. Defaults to `AWS_SECRET_ACCESS_KEY` | YES |
...
| `DB_PASS` | The database database password. Defaults to no password | Yes |
...
| `OAUTH_GOOGLE_API_KEY` | Google App Client ID. No defaults. | YES |
| `OAUTH_GOOGLE_APP_SECRET` | Google App Client Secret. No defaults. | YES |
...
| `OAUTH_FACEBOOK_API_KEY` | Facebook App API key. No defaults. | YES |
| `OAUTH_FACEBOOK_APP_SECRET` | Facebook App API secret. No defaults. | YES |
| `OAUTH_TWITTER_API_KEY` | Twitter App API key. No defaults. | YES |
| `OAUTH_TWITTER_APP_SECRET` | Twitter App API secret. No defaults. | YES |
| `OAUTH_AUTHENTIQ_CLIENT_ID` | authentiq Client ID. No defaults. | YES |
| `OAUTH_AUTHENTIQ_CLIENT_SECRET` | authentiq Client secret. No defaults. | YES |
...
| `OAUTH_GITHUB_API_KEY` | GitHub App Client ID. No defaults. | YES |
| `OAUTH_GITHUB_APP_SECRET` | GitHub App Client secret. No defaults. | YES |
...
| `OAUTH_GITLAB_API_KEY` | GitLab App Client ID. No defaults. | YES |
| `OAUTH_GITLAB_APP_SECRET` | GitLab App Client secret. No defaults. | YES |
| `OAUTH_BITBUCKET_API_KEY` | BitBucket App Client ID. No defaults. | YES |
| `OAUTH_BITBUCKET_APP_SECRET` | BitBucket App Client secret. No defaults. | YES |
...
| `OAUTH_CROWD_APP_PASSWORD` | Crowd server application password. No defaults. | YES |
| `OAUTH_AUTH0_CLIENT_ID` | Auth0 Client ID. No defaults. | YES |
| `OAUTH_AUTH0_CLIENT_SECRET` | Auth0 Client secret. No defaults. | YES |
| `OAUTH_AUTH0_DOMAIN` | Auth0 Domain. No defaults. | |
| `OAUTH_AZURE_API_KEY` | Azure Client ID. No defaults. | YES |
| `OAUTH_AZURE_API_SECRET` | Azure Client secret. No defaults. | YES |
...
| `AWS_BACKUP_ACCESS_KEY_ID` | AWS access key id. No defaults. | YES |
| `AWS_BACKUP_SECRET_ACCESS_KEY` | AWS secret access key. No defaults. | YES |
...
| `GCS_BACKUP_ACCESS_KEY_ID` | GCS access key id. No defaults | YES |
| `GCS_BACKUP_SECRET_ACCESS_KEY` | GCS secret access key. No defaults | YES |
