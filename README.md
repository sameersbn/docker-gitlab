...
| `GITLAB_PAGES_NGINX_DISABLE` | Disable the nginx proxy for gitlab pages, defaults to `false`. When set to `true` this will turn off the nginx proxy to the gitlab pages daemon, used when the user provides their own http load balancer in combination with a gitlab pages custom domain setup. |
