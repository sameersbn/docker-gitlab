...
```bash
docker pull sameersbn/gitlab:8.2.1
```
...
docker run --name gitlab -d [OPTIONS] \
    sameersbn/gitlab:8.2.1 app:sanitize
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:8.2.1 app:rake gitlab:backup:create
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:8.2.1 app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    sameersbn/gitlab:8.2.1 app:rake gitlab:backup:restore BACKUP=1417624827
```
...
```bash
docker run --name gitlab -d [OPTIONS] \
    sameersbn/gitlab:8.2.1 app:rake gitlab:env:info
```
...
```bash
docker run --name gitlab -d [OPTIONS] \
    sameersbn/gitlab:8.2.1 app:rake gitlab:import:repos
```
...
```bash
docker run --name gitlab -d [OPTIONS] sameersbn/gitlab:8.2.1
```
