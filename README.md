...
```bash
docker pull sameersbn/gitlab:7.4.0
```
...
```bash
docker run --name=gitlab -it --rm [OPTIONS] \
  sameersbn/gitlab:7.4.0 app:rake gitlab:backup:restore
```
...
```bash
docker run --name=gitlab -d [OPTIONS] sameersbn/gitlab:7.4.0
```
...
docker run --name=gitlab -d [OPTIONS] \
  sameersbn/gitlab:7.4.0 app:rake gitlab:env:info
```
...
```bash
docker run --name=gitlab -d [OPTIONS] \
  sameersbn/gitlab:7.4.0 app:rake gitlab:import:repos
```
